...
for(m in mvals){
  ...
  lauxtime <- microbenchmark(init_bi$run(add_options = 
                                           paste("--init-file initvars.nc 
                                                 --filter lookahead 
                                                 --nparticles ", m, sep="")),
                             times=10, unit='s')
...
init_bi$run(add_options = paste("--init-file initvars.nc
                                --filter lookahead  --nparticles ", m, sep=""))

plotmat <- data.frame(value = c(as.matrix(cmodAux$mv, 'xs'),
                                bi_read_var(init_bi$result$output_file_name, "x")[100,]),
                      package=c(rep("Nimble",m),rep("LibBi",m)),
                      wtz= c(as.matrix(cmodAux$mv, 'wts'), 
                             exp(bi_read_var(init_bi$result$output_file_name, "logweight")[100,])))
...
```{r biipstable, echo=FALSE, results='hide', message=FALSE, cache=TRUE}
library(Rbiips)
...
for(m in mvals){
  nboottime <- microbenchmark(cmodBoot$run(m), times=10, unit="s")
  nbootlik <- mean(sapply(rep(m,10),function(x)cmodBoot$run(x)))
  bboottime <- microbenchmark(biips_smc_samples(bmod, 'x[1:100]', m, type = "f",
                                                rs_thres=1 ),
                              times=10, unit="s")
  bbootlike <- mean(sapply(rep(m,10),function(x)bipLikeFunc(x)))
  timelist <- rbind(timelist,cbind("Method" = c("NIMBLE Bootstrap"
                                                , "Biips Bootstrap"),
                                   "N. Particles" =c(m,m),
                                   "Mean Log Lik." = c(nbootlik, bbootlike),
                                   rbind(summary(nboottime), 
                                         summary(bboottime))))
}
...
cmodBoot$run(m)
biipsOut <- biips_smc_samples(bmod, 'x[1:100]', m, type = "f", rs_thres=1 )

plotmat <- data.frame(value = c(as.matrix(cmodBoot$mv, 'xs'),
                                biipsOut$x$f$values[100,]),
                      package=c(rep("Nimble",m),rep("Biips",m)),
                      wtz= c(rep(1,m), biipsOut$f$weights[100,]))
